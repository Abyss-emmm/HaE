...
//    private SetConfig setConfig = new SetConfig();
    ...
    private Map<String, Rules> ruleConfig = Config.getInstance().getRules();
    ...
    private void ruleAddMouseClicked(MouseEvent e, JTabbedPane pane) {
        ...
        if(isOk == 0){
            ...
            Rule rule = new Rule();
            rule.setLoaded(false);
            rule.setName(add.Name.getText());
            rule.setRegex(add.Regex.getText());
            rule.setColor(add.ColorSelect.getSelectedItem().toString());
            rule.setScope(add.ScopeSelect.getSelectedItem().toString());
            rule.setEngine(add.EngineSelect.getSelectedItem().toString());
            this.model.insertRow(this.model.getRowCount(),rule.getRuleObject());
            ruleConfig.get(pane.getTitleAt(pane.getSelectedIndex())).rule.add(rule);
            config.saveConfig();
    ...
    private void ruleEditMouseClicked(MouseEvent e, JTabbedPane pane){
        if (jTable.getSelectedRowCount() >= 1){
            ...
            if (isOk == 0){
                ...
                Rule rule = ruleConfig.get(pane.getTitleAt(pane.getSelectedIndex())).rule.get(select);
                rule.setName(edit.Name.getText());
                rule.setRegex(edit.Regex.getText());
                rule.setColor(edit.ColorSelect.getSelectedItem().toString());
                rule.setScope(edit.ScopeSelect.getSelectedItem().toString());
                rule.setEngine(edit.EngineSelect.getSelectedItem().toString());
                config.saveConfig();
//                model = (DefaultTableModel) jTable.getModel();
    ...
    private void ruleRemoveMouseClicked(MouseEvent e, JTabbedPane pane){
        if (jTable.getSelectedRowCount() >= 1){
            ...
            if (isOk == 0){
                ...
                ruleConfig.get(pane.getTitleAt(pane.getSelectedIndex())).rule.remove(select);
                config.saveConfig();
//                model = (DefaultTableModel) jTable.getModel();
//                setConfig.remove(select, pane.getTitleAt(pane.getSelectedIndex()));
    ...
    private void ruleTableChange(TableModelEvent e, JTabbedPane pane) {
        if (e.getColumn() == 0 && jTable.getSelectedRow() != -1 && !isEdit){
            model = (DefaultTableModel) jTable.getModel();
            int select = jTable.convertRowIndexToModel(jTable.getSelectedRow());
            boolean loaded = (boolean)model.getValueAt(select,0);
            ruleConfig.get(pane.getTitleAt(pane.getSelectedIndex())).rule.get(select).setLoaded(loaded);
            config.saveConfig();
//            setConfig.edit((Vector) model.getDataVector().get(select), select, pane.getTitleAt(pane.getSelectedIndex()));
        }
    }
